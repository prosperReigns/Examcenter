...
// Fetch student info
$stmt = $db->prepare("SELECT full_name, class FROM students WHERE id = ?");
...
// Fetch scores
$stmt = $db->prepare("
    SELECT s.subject_name, ss.ca1, ss.ca2, ss.ca3, ss.ca4, ss.exam
...
// Fetch remark
$stmt = $db->prepare("SELECT remark FROM student_term_remarks WHERE student_id = ? AND academic_year_id = ? LIMIT 1");
$stmt->bind_param('ii', $student_id, $academic_year_id);
$stmt->execute();
...
$html .= '<p>Name: ' . htmlspecialchars($student['full_name']) . '</p>';
...
foreach ($scores as $row) {
    $total = array_sum([$row['ca1'], $row['ca2'], $row['ca3'], $row['ca4'], $row['exam']]);
    $totalSum += $total;
    $html .= "<tr>
