<?php
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT username, password, role FROM super_admins WHERE id = ?");
        if (!$stmt) {
        ...
        if (!$super_admin || strtolower($super_admin['role']) !== 'super_admin') {
            error_log("Unauthorized access attempt by user_id=$user_id, role=" . ($super_admin['role'] ?? 'none'));
            session_destroy();
        ...
        $activity = "Admin {$super_admin['username']} accessed settings page.";
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['change_password'])) {
        ...
        if (empty($current_password) || empty($new_password) || empty($confirm_password)) {
            ...
            try {
                ...
                // Log activity
                $activity = "Super_admin {$super_admin['username']} changed their password.";
                $stmt = $conn->prepare(
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_settings'])) {
        ...
        try {
            ...
            // Log activity
            $activity = "Admin {$super_admin['username']} updated system settings: show_results_immediately={$show_results}";
            $stmt = $conn->prepare(
                "INSERT INTO activities_log (activity, admin_id, ip_address, user_agent, created_at) VALUES (?, ?, ?, ?, NOW())"
            );
            $stmt->bind_param("siss", $activity, $user_id, $ip_address, $user_agent);
   ...
   // --- Handle Daily Subjects Update ---
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_daily_subjects'])) {
        ...
        if (empty($exam_date) || !preg_match('/^\d{4}-\d{2}-\d{2}$/', $exam_date)) {
            ...
            try {
                ...
                // Log activity
                $activity = "Super_admin {$super_admin['username']} updated daily subjects for {$exam_date}: " . implode(', ', $valid_subjects);
                $stmt = $conn->prepare(
                    "INSERT INTO activities_log (activity, admin_id, ip_address, user_agent, created_at) VALUES (?, ?, ?, ?, NOW())"
                );
                $stmt->bind_param("siss", $activity, $user_id, $ip_address, $user_agent);
    ...
    <head>
        ...
        <title>System Settings | Examcenter</title>
